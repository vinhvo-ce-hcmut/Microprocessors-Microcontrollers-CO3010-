...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Lab 01 - Exercise 9 (all clock LEDs on, one LED off per step)
  *
  * Wiring:
...
static void turnAllClockOn(void);
static void runExercise9(void);

static void clearNumberOnClock(int num)
{
  if ((num >= 0) && (num <= 11))
  {
    HAL_GPIO_WritePin(GPIOA, (uint16_t)(GPIO_PIN_4 << num), LED_OFF);
  }
}

static void turnAllClockOn(void)
{
  HAL_GPIO_WritePin(GPIOA, CLOCK_LED_MASK, LED_ON);
}
...
static void runExercise9(void)
{
  turnAllClockOn();
  HAL_Delay(250U);
  clearNumberOnClock((int)counter);
  HAL_Delay(750U);
